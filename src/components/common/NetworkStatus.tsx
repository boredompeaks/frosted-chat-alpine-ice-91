...
const NetworkStatus: React.FC<NetworkStatusProps> = ({
  ...
  const checkConnectivity = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const isConnected = results.some(
      ...
      if (isConnected) {
        ...
        if (state.retryCount < maxRetries && autoReconnect) {
          retryTimeoutRef.current = setTimeout(
            ...
            retryInterval * Math.pow(1.5, state.retryCount),
          ); // Exponential backoff
        }
      }
    } catch (error) {
  ...
  if (variant === "indicator") {
    return (
      <div className={`flex items-center ${className}`} title={`Network status: ${state.isOnline ? "Online" : "Offline"}`}>
        <motion.div
          className={`w-2.5 h-2.5 rounded-full ${
            state.isOnline ? "bg-green-400" : "bg-red-400"
          }`}
          animate={state.isOnline ? { opacity: [0.5, 1, 0.5], scale: [1, 1.2, 1] } : {}}
          transition={state.isOnline ? { duration: 2, repeat: Infinity } : {}}
        />
      </div>
    );
  }
  ...
  return (
    <div className={`glass-card p-3 ${className}`}>
      <div className="flex items-center justify-between">
          ...
          <div>
            ...
            {state.isOnline && state.connectionType !== "unknown" && (
              <div className="flex items-center space-x-2 text-xs text-white/60">
                ...
                {state.rtt > 0 && <span>{state.rtt}ms</span>}
              </div>
            )}
        ...
        {state.isOnline && (
          <div
            className={`w-2 h-2 rounded-full ${qualityColors[networkQuality].replace("text-", "bg-")}`}
          />
        )}
      </div>
      ...
      {!state.isOnline && state.retryCount < maxRetries && (
        <div className="mt-3 pt-3 border-t border-white/10">
          ...
          >
            ...
            {isRetrying
              ? "Reconnecting..."
              : `Reconnect (${state.retryCount}/${maxRetries})`}
          </button>
        </div>
      )}
    </div>
